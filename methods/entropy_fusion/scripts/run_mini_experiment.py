"""Run a bounded mini experiment with scheduled official validation and best checkpoint."""
import argparse
...
def main():
    ...
    try:
        for stage in stages:
            ...
            command = ['train.py', '--epochs', str(stage), '--output', str(output/'training')]
            ...
            run(['eval.py', '--checkpoint', str(checkpoint), '--output', str(eval_dir)], output/f'eval_epoch_{stage:02}.log')
